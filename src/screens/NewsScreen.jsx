...
import { useState as useInfoState } from 'react'
...
function NewsCard({ T, item, big }) {
  ...
  return (
    <motion.div
      {...TAP}
      onClick={() => tappable && window.open(item.url, '_blank')}
      style={{
    ...
    >
      {big && <div style={{ height: 3, background: item.color || T.pr, position: 'absolute', top: 0, left: 0, right: 0 }} />}
      <div style={{ paddingTop: big ? 8 : 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: big ? 8 : 5 }}>
          <span style={{
            fontSize: 13, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase',
            color: item.color || T.pr, background: `${item.color || T.pr}1e`,
            borderRadius: 4, padding: '2px 6px',
          }}>
            {item.category}
          </span>
          <span style={{ fontSize: 13, fontWeight: 500, color: T.tl }}>{item.time}</span>
        </div>
        <div style={{ fontSize: big ? 15 : 12, fontWeight: 700, color: T.th, lineHeight: 1.45, marginBottom: 5 }}>
          {item.title}
        </div>
        <div style={{ fontSize: 13, fontWeight: 600, color: T.tl }}>
          {item.source}{tappable ? ' ↗' : ''}
        </div>
      </div>
    </motion.div>
  )
...
function NewsInfoSheet({ T, onClose }) {
  return (
    <div style={{ position:'fixed', inset:0, zIndex:500, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'flex-end' }}
      ...
      <div style={{ background:T.sf, borderRadius:'20px 20px 0 0', width:'100%', maxHeight:'90dvh', overflowY:'auto',
        padding:'20px 18px calc(32px + env(safe-area-inset-bottom))' }}>
        <div style={{ width:36, height:4, borderRadius:999, background:T.tl, opacity:0.3, margin:'0 auto 20px' }}/>
        <div style={{ fontSize:20, fontWeight:800, color:T.th, marginBottom:4 }}>About this news feed</div>
        <div style={{ fontSize:14, fontWeight:500, color:T.tl, marginBottom:16 }}>How it works and what we include</div>

        <div style={{ borderRadius:14, padding:'14px', background:T.c0, border:`1px solid ${T.cardBorder||'rgba(0,0,0,0.08)'}`, marginBottom:12 }}>
          <div style={{ fontSize:13, fontWeight:800, color:T.pr, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8 }}>How stories are ranked</div>
          <div style={{ fontSize:14, fontWeight:500, color:T.tm, lineHeight:1.7 }}>
            Stories are ranked by how many outlets cover them. If BBC, GB News, Guardian and Sky News all run the same story, it scores highly. A story only one outlet covers ranks lower. Recency breaks ties. No human editorial decisions are made.
          </div>
        </div>

        <div style={{ fontSize:13, fontWeight:800, color:T.tl, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10 }}>
          Sources ({NEWS_SOURCES.length})
        </div>
        {NEWS_SOURCES.map((s, i) => (
          <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 0',
            borderBottom: i < NEWS_SOURCES.length-1 ? `1px solid ${T.cardBorder||'rgba(0,0,0,0.06)'}` : 'none' }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                ...
                <span style={{ fontSize:12, fontWeight:700, padding:'2px 7px', borderRadius:99,
                  background:LEAN_COLOR[s.lean]+'18', color:LEAN_COLOR[s.lean] }}>{s.lean}</span>
              </div>
        ...
        <div style={{ borderRadius:14, padding:'14px', background:T.c0, border:`1px solid ${T.cardBorder||'rgba(0,0,0,0.08)'}`, marginTop:12, marginBottom:16 }}>
          <div style={{ fontSize:13, fontWeight:800, color:T.tl, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8 }}>Why no Telegraph, Mail, Times?</div>
          <div style={{ fontSize:14, fontWeight:500, color:T.tm, lineHeight:1.7 }}>
            The Telegraph, Daily Mail and Times actively block automated server requests — a technical decision on their end. X/Twitter restricted API access in 2023 at ~$100/month. Not viable for a free app.
          </div>
        </div>

        <button onClick={onClose} style={{ width:'100%', padding:'14px', borderRadius:999, border:'none',
          background:T.c1||'rgba(0,0,0,0.07)', color:T.th, fontSize:15, fontWeight:700, cursor:'pointer',
          fontFamily:'Outfit,sans-serif' }}>Got it</button>
      </div>
    </div>
  )
}
...
export function NewsScreen({ T, nav }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: T.sf }}>
      <PushHeader back={() => nav('home')} title="News" T={T}
        right={<button onClick={() => setInfoOpen(true)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:22, padding:'4px 8px', color:T.tl }}>ℹ</button>}
      />
      {infoOpen && <NewsInfoSheet T={T} onClose={() => setInfoOpen(false)} />}
      <StickyPills pills={NEWS_TABS} active={tab} onSelect={setTab} T={T} />
      <ScrollArea>
        <div style={{ padding: '4px 14px' }}>
          {filtered[0] && <NewsCard T={T} item={filtered[0]} big={true} />}
          {filtered.slice(1).map((item, i) => <NewsCard key={i} T={T} item={item} big={false} />)}
          <div style={{ textAlign: 'center', padding: '10px 0 16px', fontSize: 13, fontWeight: 500, color: T.tl }}>
