...
const DATA = {
  ...
  scotland: {
    ...
    groups:[
      {label:'SNP',    REF:null, LAB:null, CON:null, GRN:null, LD:null, SNP:36},
      {label:'Labour', REF:null, LAB:28,   CON:null, GRN:null, LD:null},
      {label:'Con',    REF:null, LAB:null, CON:14,   GRN:null, LD:null},
      {label:'Reform', REF:14,   LAB:null, CON:null, GRN:null, LD:null},
      {label:'Green',  REF:null, LAB:null, CON:null, GRN:10,   LD:null},
      {label:'Lib Dem',REF:null, LAB:null, CON:null, GRN:null, LD:8   },
    ],
    customRows: [
          ...
          {parties
            ...
            .map((p, pi) => (
              <div key={pi} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:5 }}>
                <div style={{ width:28, fontSize:12, fontWeight:700, color:p.color, flexShrink:0 }}>{p.label}</div>
                <div style={{ flex:1, height:7, borderRadius:999, background:T.c1||'rgba(0,0,0,0.07)', overflow:'hidden' }}>
                  <div style={{ width:`${Math.min(100,(g[p.key]||0)/maxScale*100)}%`, height:'100%', background:p.color, borderRadius:999 }}/>
                </div>
                <div style={{ fontSize:13, fontWeight:800, color:p.color, width:34, textAlign:'right', flexShrink:0 }}>{g[p.key]}%</div>
              </div>
            ))
          }
        </div>
    ...
    <div>
      {rows.map((r, i) => (
        <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 0',
          ...
          <div style={{ flex:1, height:8, borderRadius:999, background:T.c1||'rgba(0,0,0,0.07)', overflow:'hidden' }}>
            <div style={{ width:`${(r.val/max)*100}%`, height:'100%', background:r.color, borderRadius:999 }}/>
          </div>
          <div style={{ fontSize:14, fontWeight:800, color:r.color, width:36, textAlign:'right', flexShrink:0 }}>{r.val}%</div>
        </div>
      ))}
    </div>
  )
...
export default function DemographicsScreen({ T, nav }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden', background:T.sf }}>
      <PushHeader back={() => nav('home')} title="Demographics" T={T}/>
      <StickyPills pills={TABS} active={tab} onSelect={setTab} T={T}/>

      <ScrollArea>
        <div style={{ padding:'8px 14px 48px' }}>

          <div style={{ borderRadius:14, padding:'16px', background:T.c0, border:`1px solid ${T.cardBorder||'rgba(0,0,0,0.08)'}`, marginBottom:10 }}>
            <div style={{ fontSize:17, fontWeight:800, color:T.th, marginBottom:6 }}>{d.title}</div>
            <div style={{ fontSize:14, fontWeight:500, color:T.tm, lineHeight:1.65, marginBottom: (d.groups||d.customRows) ? 14 : 0 }}>{d.note}</div>

            {/* Standard bar groups */}
            {d.groups && <BarGroup groups={d.groups} parties={PARTIES} T={T}/>}

            {/* Custom rows (Scotland, Wales) */}
            {d.customRows && !d.groups && <SimpleBar rows={d.customRows} T={T}/>}

            {/* Non-voters special layout */}
            {tab === 'nonvoters' && (
              <>
                <div style={{ fontSize:14, fontWeight:800, color:T.tl, textTransform:'uppercase', letterSpacing:'0.06em', marginTop:16, marginBottom:10 }}>
                  Key stats
                </div>
                {d.facts.map((f, i) => (
