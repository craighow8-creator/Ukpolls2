...
const DIFF_COLORS = {
  'very hard': '#E4003B', hard: '#F97316', medium: '#EAB308', safe: '#02A95B',
}
...
function ControlBadge({ control, T }) {
  ...
  return (
    <span style={{ fontSize: 13, fontWeight: 800, padding: '2px 8px', borderRadius: 999, background: `${c}1e`, color: c, flexShrink: 0 }}>
      {control}
    </span>
  )
}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 10 }}>
      {polls.map((p, i) => (
        <div key={i} style={{
          ...
          <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.02em', color: T.th, lineHeight: 1 }}>{p.pct}%</div>
...
export default function ElectionsScreen({ T, nav, meta }) {
  ...
  const filtered = useMemo(() => {
    ...
    return allSearchableCouncils.filter(c =>
  ...
  const openCouncil = (name) => { haptic(6); nav('council', { name }) }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: T.sf }}>

      <PushHeader back={() => nav('home')} title="Elections" T={T}/>

      {/* Countdown + chips */}
      <div style={{ padding: '10px 18px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 20, fontWeight: 800, color: T.th, lineHeight: 1 }}>{new Date(nextDate).toLocaleDateString('en-GB', { day:'numeric', month:'long', year:'numeric' })}</div>
            <div style={{ fontSize: 13, fontWeight: 500, color: T.tl, marginTop: 2 }}>{nextLabel}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 36, fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1, color: T.pr }}>{days}</div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
          {['136 English councils', 'Scottish Parliament', 'Senedd', 'Mayoral'].map((s, i) => (
            <Chip key={i} color={T.pr || '#12B7D4'}>{s}</Chip>
          ))}
        </div>
        {meta?.fetchDate && (
        ...
        <div style={{ padding: '4px 14px' }}>
          ...
          {tab === 'overview' && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 12 }}>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    background: T.c0 || '#fff', border: `1px solid ${s.color}28`,
                  }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: T.tl, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>{s.label}</div>
              ...
              {councils.filter(c => c.difficulty === 'very hard').slice(0, 6).map((c, i) => (
                <motion.div
                  key={i} {...TAP}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, color: T.tl, marginBottom: 5 }}>{c.region} · {c.seats} seats</div>
                    {c.watchFor && (
                      <div style={{ fontSize: 13, fontWeight: 500, color: T.tm, lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {c.watchFor}
                      </div>
                    )}
                    {c.verdict && (
                      <div style={{ fontSize: 13, fontWeight: 700, color: DIFF_COLORS[c.difficulty] || T.tl, marginTop: 4 }}>{c.verdict}</div>
                    )}
                  </div>
          ...
          {tab === 'councils' && (
            <>
              ...
              <div style={{ position: 'relative', marginBottom: 10 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    borderRadius: 12, fontSize: 15, color: T.th,
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: T.tl, marginBottom: 8 }}>
                {filtered.length} of {councils.length} councils {search ? '· tap to clear search' : '· tap for full profile'}
              </div>
              {filtered.map((c, i) => (
                <motion.div
                  key={i} {...TAP}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: T.tl }}>{c.region} · {c.type} · {c.seats} seats</div>
                    {c.watchFor && (
                      <div style={{ fontSize: 13, color: T.tl, marginTop: 2, display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {c.watchFor}
                      </div>
                    )}
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <ControlBadge control={c.control} T={T} />
                    {c.verdict && (
                      <div style={{ fontSize: 13, fontWeight: 700, color: DIFF_COLORS[c.difficulty] || T.tl, marginTop: 3 }}>{c.verdict}</div>
                    )}
                  </div>
                </motion.div>
              ))}
            ...
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: T.tl }}>{r.councils} councils · {r.seats} seats</div>
                </div>
              ...
              {r.watchFor && (
                <div style={{ padding: '8px 10px', background: T.c1 || 'rgba(0,0,0,0.04)', borderRadius: 8, marginBottom: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.10em', textTransform: 'uppercase', color: T.tl, marginBottom: 3 }}>Watch for</div>
              ...
              {r.councils_list?.length > 0 && (
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {r.councils_list.map((cl, j) => (
                    <motion.div key={j} {...TAP} onClick={() => openCouncil(cl.name)}
                      style={{ fontSize: 13, fontWeight: 700, padding: '3px 9px', borderRadius: 999, background: T.c1 || 'rgba(0,0,0,0.05)', color: T.th, cursor: 'pointer', border: `1px solid ${T.cardBorder || 'rgba(0,0,0,0.07)'}` }}>
                      {cl.name}
                    </motion.div>
                  ))}
                </div>
          ...
          {tab === 'scotland' && (
            <>
              <div style={{ borderRadius: 14, padding: '13px 14px', marginBottom: 10, background: T.c0 || '#fff', border: `1px solid #C4922A28` }}>
                <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#C4922A', marginBottom: 10 }}>
                  🏴󠁧󠁢󠁳󠁣󠁴󠁿 Scottish Parliament · 7 May 2026
                </div>
              ...
              {[
                ...
                <div key={i} style={{ borderRadius: 12, padding: '10px 12px', marginBottom: 6, background: T.c0 || '#fff', border: `1px solid ${T.cardBorder || 'rgba(0,0,0,0.07)'}` }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: T.th, marginBottom: 2 }}>{s.seat}</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: T.tl, marginBottom: 3 }}>{s.holding} → {s.challenger}</div>
          ...
          {tab === 'wales' && (
            <>
              <div style={{ borderRadius: 14, padding: '13px 14px', marginBottom: 10, background: T.c0 || '#fff', border: `1px solid #3F842828` }}>
                <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#3F8428', marginBottom: 10 }}>
                  🏴󠁧󠁢󠁷󠁬󠁳󠁿 Senedd · 7 May 2026
                </div>
                <div style={{ fontSize: 13, fontWeight: 500, color: T.tm, lineHeight: 1.65, marginBottom: 10 }}>
