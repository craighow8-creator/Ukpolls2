...
function SparkLine({ data = [], color = '#888', width = 80, height = 28, filled = false }) {
  ...
  const fill = filled
    ? `${pts[0][0]},${height} ${line} ${pts[pts.length - 1][0]},${height}`
    : null
  return (
...
function LargeCard({ T, color, onClick, children }) {
  ...
  return (
    <motion.div
      {...TAP}
      onClick={() => { if (onClick) { haptic(6); onClick() } }}
      style={{
        gridColumn: 'span 2',
        background: bg,
...
function SmallCard({ T, color, onClick, children }) {
  const isDark = T.th === '#ffffff' || T.th?.toLowerCase?.() === '#ffffff'
  const bg = isDark ? '#0d1a24' : '#ffffff'
  const borderCol = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'
  return (
...
const Lbl = ({ children, T }) => (
  <div style={{
    ...
    textTransform: 'uppercase', color: T.tl, marginBottom: 8
  }}>{children}</div>
)

const BigStat = ({ children, color, T, size = 56 }) => (
  <div style={{ fontSize: size, fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1, color: color || T.th }}>
    {children}
  </div>
)

const Stat = ({ children, color, T, size = 34 }) => (
  <div style={{ fontSize: size, fontWeight: 800, letterSpacing: '-0.02em', lineHeight: 1, color: color || T.th }}>
    {children}
  </div>
)

const Sub = ({ children, T, size = 14 }) => (
  <div style={{ fontSize: size, fontWeight: 500, color: T.tm, lineHeight: 1.4, marginTop: 6 }}>
    {children}
  </div>
)

const Cta = ({ children, T }) => (
  <div style={{ fontSize: 14, fontWeight: 600, color: T.tl, marginTop: 'auto', paddingTop: 10 }}>
    {children}
  </div>
)
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 80 80">
      <defs>
        ...
        <clipPath id="lc"><circle cx="40" cy="40" r="38" /></clipPath>
      </defs>
...
export default function HomeScreen({
  T, nav, onMenu,
  parties = [], trends = [], byElections = {}, migration = {}, betting = {}, meta = {}
}) {
  ...
  if (!safe.length) {
    return (
      <div style={{ minHeight: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: T.sf }}>
  ...
  const tr12 = (name) =>
    safeTr.map(t => t[name]).filter(v => v != null && !isNaN(v)).slice(-12)
  ...
  const allLeaders = safe
    .map(p => p._leader)
    .filter(Boolean)

  const sortedLeaders = [...allLeaders].sort(
    (a, b) => (b.net ?? -999) - (a.net ?? -999)
  )
  ...
  const alertParty = (() => {
    ...
    const f = safeTr[0], l = safeTr[safeTr.length - 1]
    let worst = null, drop = 0
    main.forEach(p => {
      const d = (f[p.name] || 0) - (l[p.name] || 0)
      if (d > drop) { drop = d; worst = { ...p, drop: +d.toFixed(1), start: f[p.name], end: l[p.name] } }
    })
    return worst
  })()

  const heroStatus = leader.change >= 4 ? 'Surging'
    : leader.change <= -4 ? 'Slipping'
      : gap >= 10 ? 'Dominant' : gap <= 3 ? 'Tight race' : 'Leading'
    ...
    <div style={{ position: 'relative', minHeight: '100%', background: T.sf }}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
          ...
          <div>
            <div style={{ fontSize: isMobile ? 20 : 24, fontWeight: 800, color: T.th, letterSpacing: -0.5, lineHeight: 1 }}>
              Politiscope
            </div>
            <div style={{ fontSize: 13, fontWeight: 600, color: T.tl, marginTop: 2, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
        ...
        <motion.button
          ...
          style={{
            ...
            justifyContent: 'center', outline: 'none', flexShrink: 0
          }}
        >
      ...
      <div style={{ position: 'relative', zIndex: 3, padding: `14px ${G}px 110px` }}>
        <div style={{
          ...
          <LargeCard T={T} color={leader.color} onClick={() => nav('polls')}>
            <div style={pL}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: isMobile ? 22 : 40, marginBottom: 14 }}>
                <div style={{ flex: 1, maxWidth: 180, textAlign: 'center' }}>
                  ...
                  <BigStat color={leader.color} T={T} size={isMobile ? 48 : 56}>
                    {leader.pct}<span style={{ fontSize: '0.4em', fontWeight: 600, verticalAlign: 'super' }}>%</span>
                  </BigStat>
                  <div
                    style={{
                      fontSize: 13,
                ...
                <div style={{ width: 1, alignSelf: 'stretch', background: T.cardBorder || 'rgba(0,0,0,0.07)', opacity: 0.8 }} />
                <div style={{ flex: 1, maxWidth: 180, textAlign: 'center' }}>
                  <Lbl T={T}>2nd Place</Lbl>
                  ...
                  <BigStat color={second.color} T={T} size={isMobile ? 48 : 56}>
                    {second.pct}<span style={{ fontSize: '0.4em', fontWeight: 600, verticalAlign: 'super' }}>%</span>
                  </BigStat>
                  <div
                    style={{
                      fontSize: 13,
                ...
                {main.slice(0, 6).map((p, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: p.color, width: 28, flexShrink: 0 }}>{p.abbr}</div>
                      <MiniBar value={p.pct} max={(main[0]?.pct || 1) + 4} color={p.color} T={T} />
                      <div style={{ fontSize: 16, fontWeight: 800, color: p.color, width: 38, textAlign: 'right', flexShrink: 0 }}>{p.pct}%</div>
                      <div style={{
                        fontSize: 13, fontWeight: 700, width: 36, textAlign: 'right', flexShrink: 0,
              ...
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: T.tl, marginBottom: 7 }}>
                MRP Seat Projection · 650
              </div>
              <div style={{ height: 10, borderRadius: 999, background: T.c1, overflow: 'hidden', display: 'flex' }}>
                {main.filter(p => p.seats).map((p, i) => (
                  <div key={i} style={{ width: `${Math.round(p.seats / 650 * 100)}%`, height: '100%', background: p.color }} />
                ))}
              </div>
          ...
          <SmallPair>
            <SmallCard T={T} color={leader.color} onClick={() => nav('parties')}>
              <div style={pS}>
                <Lbl T={T}>Parties</Lbl>
                <Stat color={leader.color} T={T}>{leader.pct}%</Stat>
                <Sub T={T}>{leader.abbr} leading UK polls</Sub>
                {(leader.change || 0) !== 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: (leader.change || 0) > 0 ? '#02A95B' : '#E4003B' }}>
            ...
            <SmallCard T={T} color={topLeader?.net >= 0 ? '#02A95B' : '#E4003B'} onClick={() => nav('leaders')}>
              <div style={pS}>
                <Lbl T={T}>Leaders</Lbl>
                <Stat color={(topLeader?.net ?? 0) >= 0 ? '#02A95B' : '#E4003B'} T={T}>
                  {topLeader
                    ? `${(topLeader.net ?? 0) >= 0 ? '+' : ''}${topLeader.net ?? '—'}`
                    : '—'}
                </Stat>
                <Sub T={T}>
                  Highest rated leader · {topLeader?.name?.split(' ').pop() || '—'}
                </Sub>
          ...
          <LargeCard T={T} color={T.th} onClick={() => nav('elections')}>
            <div style={pL}>
              <Lbl T={T}>May Elections · 7 May 2026</Lbl>

              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, marginBottom: 10 }}>
                <BigStat color={T.th} T={T} size={72}>{days7May}</BigStat>
              ...
              <div style={{ height: 8, borderRadius: 999, background: T.c1, overflow: 'hidden', marginBottom: 14 }}>
                <div style={{
                  ...
                  height: '100%', background: T.th, borderRadius: 999
                }} />
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                ...
                ].map((c, i) => <Chip key={i} color={c.color}>{c.label}</Chip>)}
              </div>
          ...
          <SmallPair>
            <SmallCard T={T} onClick={() => nav('elections')}>
              <div style={pS}>
                ...
                {recBE[0] ? (
                  <>
                    <Stat color={recBE[0].winnerColor || T.pr} T={T} size={22}>{recBE[0].winner?.split(' ')[0]}</Stat>
                    <Sub T={T}>{recBE[0].gainLoss} · {recBE[0].name?.split(' ').slice(0, 2).join(' ')}</Sub>
                  </>
                ) : upBE[0] ? (
                  <>
                    <Stat color={T.pr} T={T} size={16}>Next up</Stat>
            ...
            <SmallCard T={T} color="#02A95B" onClick={() => nav('demographics')}>
              <div style={pS}>
                <Lbl T={T}>Demographics</Lbl>
                <Stat color="#02A95B" T={T}>32%</Stat>
          ...
          <LargeCard T={T} color={null} onClick={() => nav('trends')}>
            <div style={pL}>
                ...
                {main.slice(0, 5).map((p, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: p.color, width: 30, flexShrink: 0 }}>{p.abbr}</div>
                      <div style={{ flex: 1 }}>
          ...
          <SmallPair>
            <SmallCard T={T} color={topBet?.color || T.pr} onClick={() => nav('betting')}>
              <div style={pS}>
                <Lbl T={T}>Betting</Lbl>
                <Stat color={topBet?.color || T.pr} T={T}>{topBet?.odds || '6/4'}</Stat>
                ...
                {winProb != null && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, color: topBet?.color || T.pr, flexShrink: 0 }}>{winProb}%</span>
                  </div>
                )}
          ...
          <LargeCard T={T} color={alertParty ? '#E4003B' : null} onClick={() => nav('news')}>
            <div style={pL}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <Lbl T={T}>News</Lbl>
                <Chip color={T.pr}>Live</Chip>
              </div>
              <div style={{ fontSize: 20, fontWeight: 700, color: T.th, lineHeight: 1.3 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                ...
                {(meta?.headlines || []).slice(0, 2).map((h, i) => (
                  <div key={i} style={{ fontSize: 14, color: T.tm, fontWeight: 500, lineHeight: 1.35 }}>· {h}</div>
                ))}
                {!meta?.headlines?.length && (
                  <div style={{ fontSize: 13, color: T.tl }}>Open the news feed for the latest political coverage</div>
                )}
              </div>
          ...
          <SmallPair>
            <SmallCard T={T} color={T.pr} onClick={() => nav('vote')}>
              <div style={pS}>
                <Lbl T={T}>Your Vote</Lbl>
                <Stat color={T.pr} T={T} size={36}>→</Stat>
                <Sub T={T}>Who would you vote for today?</Sub>
                <Cta T={T}>Join community poll →</Cta>
              </div>
            </SmallCard>

            <SmallCard T={T} color={leader.color} onClick={() => nav('compare')}>
              <div style={pS}>
                <Lbl T={T}>Compare</Lbl>
                <Stat color={leader.color} T={T}>+{gap}pt</Stat>
                <Sub T={T}>{leader.abbr} lead over {second.abbr}</Sub>
                <div style={{ fontSize: 13, color: T.tl, marginTop: 5 }}>
          ...
          <LargeCard T={T} color={leader.color} onClick={() => nav('swingcalc')}>
            <div style={pL}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, marginBottom: 14 }}>
                <BigStat color={leader.color} T={T} size={64}>+{gap}pt</BigStat>
                <div style={{ paddingBottom: 6 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ flex: 1, height: 10, borderRadius: 999, background: T.c1, overflow: 'hidden', display: 'flex' }}>
                  <div style={{ width: `${(leader.pct / ((leader.pct + second.pct) || 1)) * 100}%`, height: '100%', background: leader.color }} />
                  <div style={{ width: `${(second.pct / ((leader.pct + second.pct) || 1)) * 100}%`, height: '100%', background: second.color }} />
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: second.color, width: 30, textAlign: 'right' }}>{second.abbr}</div>
              </div>
          ...
          <SmallPair>
            <SmallCard T={T} color={T.pr} onClick={() => nav('quotematch')}>
              <div style={pS}>
                <Lbl T={T}>Daily Game</Lbl>
                <Stat color={T.pr} T={T} size={32}>🎯</Stat>
                <Sub T={T}>Guess who said it</Sub>
                <Cta T={T}>Play · resets midnight →</Cta>
              </div>
            </SmallCard>

            <SmallCard T={T} color="#C8102E" onClick={() => nav('parliament')}>
              <div style={pS}>
                <Lbl T={T}>Parliament</Lbl>
                <Stat color="#C8102E" T={T} size={32}>📺</Stat>
          ...
          <SmallPair>
            <SmallCard T={T} color="#012169" onClick={() => nav('seatbuilder')}>
              <div style={pS}>
                <Lbl T={T}>Seat Builder</Lbl>
                <Stat color="#012169" T={T} size={32}>🏛️</Stat>
                <Sub T={T}>Set vote shares · project seats</Sub>
                <Cta T={T}>Build your parliament →</Cta>
              </div>
            </SmallCard>
            <SmallCard T={T} color="#02A95B" onClick={() => nav('pollpredictor')}>
              <div style={pS}>
                <Lbl T={T}>Predictor</Lbl>
                <Stat color="#02A95B" T={T} size={32}>🎯</Stat>
