...
const POLL_MAP = {
  ...
  'Green': 16, 'Lib Dem': 12, 'Restore Britain': 7, 'SNP': 3,
}
...
export default function VoteScreen({ T, nav, meta }) {
  ...
  const loadVotes = async () => {
    try {
      const r    = await fetch(`${WORKER}/votes`, { headers: { 'X-App-Token': APP_TOKEN } })
  ...
  const castVote = async (party) => {
    ...
    try {
      await fetch(`${WORKER}/vote`, {
        ...
        body: JSON.stringify({ party, deviceId: getDeviceId(), prevVote: prev }),
      })
    } catch {}
    await loadVotes()
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: T.sf }}>
      <div style={{ padding: '20px 18px 0', flexShrink: 0 }}>
        <div style={{ fontSize: 28, fontWeight: 800, letterSpacing: -0.8, color: T.th, lineHeight: 1 }}>Your Vote</div>
        <div style={{ fontSize: 13, fontWeight: 500, color: T.tl, marginTop: 3 }}>
          Cast your vote · see how app users compare to polls
        </div>
      </div>
      ...
      <ScrollArea>
        <div style={{ padding: '12px 14px' }}>
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    gap: 3, padding: '12px 8px', textAlign: 'center',
                  }}
                >
                  <div style={{ height: 3, background: p.color, width: '100%', position: 'absolute', top: 0, left: 0 }} />
                  <div style={{ fontSize: 14, fontWeight: 800, color: sel ? p.color : T.th }}>{p.name}</div>
                  {sel
                    ? <div style={{ fontSize: 13, fontWeight: 800, color: p.color }}>✓ Your vote</div>
                    : pollPct ? <div style={{ fontSize: 13, fontWeight: 700, color: T.tl }}>Poll: {pollPct}%</div>
                    : null
                  }
                </motion.div>
              )
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7, marginBottom: 16 }}>
            {otherParties.map((p, i) => {
              ...
              return (
                ...
                >
                  <div style={{ fontSize: 13, fontWeight: 800, color: sel ? p.color : T.th }}>{p.name}</div>
          ...
          {total > 0 && (
            <>
              <div style={{ textAlign: 'center', marginBottom: 14 }}>
                <div style={{ fontSize: 44, fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1, color: T.pr }}>
                  {total.toLocaleString()}
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: T.tl, marginTop: 3, textTransform: 'uppercase', letterSpacing: '0.10em' }}>
                  Total votes cast
                </div>
              </div>

              {PARTIES_VOTE
                .filter(p => (votes[p.name] || 0) > 0 || POLL_MAP[p.name])
                  ...
                  return (
                    <div key={i} style={{ marginBottom: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        ...
                        {n > 0 && pollPct > 0 && (
                          <span style={{ fontSize: 13, fontWeight: 800, padding: '2px 7px', borderRadius: 999, background: `${p.color}16`, color: p.color }}>
                      ...
                      {n > 0 && (
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 3 }}>
                          ...
                          <div style={{ flex: 1, height: 10, borderRadius: 999, background: T.c1 || 'rgba(0,0,0,0.07)', overflow: 'hidden' }}>
                            <div style={{ width: `${userPct}%`, height: '100%', background: p.color, borderRadius: 999 }} />
                          </div>
                          <div style={{ fontSize: 13, fontWeight: 800, color: p.color, width: 32, textAlign: 'right' }}>{userPct}%</div>
                        </div>
                      )}
                      {pollPct > 0 && (
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: T.tl, width: 28 }}>Poll</div>
                          <div style={{ flex: 1, height: 6, borderRadius: 999, background: T.c1 || 'rgba(0,0,0,0.07)', overflow: 'hidden' }}>
                            <div style={{ width: `${pollPct}%`, height: '100%', background: `${p.color}66`, borderRadius: 999 }} />
                          </div>
                          <div style={{ fontSize: 13, fontWeight: 600, color: T.tl, width: 32, textAlign: 'right' }}>{pollPct}%</div>
                        </div>
                      )}
                    </div>
                  )
          ...
          <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: T.tl, marginBottom: 8, marginTop: 6 }}>
            Poll of the day
          </div>
          <POTDWidget T={T} />
