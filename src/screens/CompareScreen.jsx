  ...
  return (
    ...
    >
      ...
      >
        ...
        {winnerA === true && (
          <div style={{ fontSize: 12, fontWeight: 800, color: colorA, marginTop: 2 }}>HIGHER</div>
        )}
      </div>
      ...
      >
        ...
        {winnerA === false && (
          <div style={{ fontSize: 12, fontWeight: 800, color: colorB, marginTop: 2 }}>HIGHER</div>
        )}
      </div>
    </div>
  )
...
export default function CompareScreen({ T, nav, parties = [], leaders = [] }) {
  ...
  const availTabs = POLICY_TOPICS.filter((t) => {
    ...
    return leaderA?.[t.key] || leaderB?.[t.key] || pledgesA?.[t.key]?.length || pledgesB?.[t.key]?.length
  })
        ...
        {both && tab === 'overview' && (
          <>
            ...
            <H2HRow
              ...
              colorA={partyA.color}
              colorB={partyB.color}
              winnerA={(partyA.seats || 0) > (partyB.seats || 0)}
            />
            <H2HRow
              T={T}
              label="Weekly"
              ...
              colorA={partyA.color}
              colorB={partyB.color}
              winnerA={partyA.change > partyB.change}
            />
            ...
            {leaderA && leaderB && (
              <H2HRow
                T={T}
                label="Leader"
                valA={`${leaderA.net >= 0 ? '+' : ''}${leaderA.net}`}
                valB={`${leaderB.net >= 0 ? '+' : ''}${leaderB.net}`}
                colorA={partyA.color}
                colorB={partyB.color}
                winnerA={leaderA.net > leaderB.net}
              />
            )}
          ...
          <>
            ...
            {(pledgesA?.[tab]?.length || pledgesB?.[tab]?.length) > 0 && (
              <>
                ...
                {Array.from({
                  length: Math.max(pledgesA?.[tab]?.length || 0, pledgesB?.[tab]?.length || 0),
                }).map((_, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 6 }}>
                      {[{ pledge: pA, party: partyA }, { pledge: pB, party: partyB }].map(
                        ({ pledge, party }, j) => (
                          <div
                            key={j}
                            style={{
